...
class VideoThread(QThread):
    ...
    def __init__(self, path_cfg = 'core/config/cfg.json'):
        ...
        self.atgt = ATGT( speed_estimate_area,speed_estimate_area, deadline, estimateKM,max_speed_motocycle, max_speed_car, max_speed_truck,0.3)
        self.atgt.init_model()
        
    def run(self):
        cam = cv2.VideoCapture( "E:/IAMDAT/Data/DOANCUOIKY/Video/91.mp4")
        time_now = 0
        if not cam.isOpened(): 
        ...
        while True:
            ret, img = cam.read()
            if ret:
                img = self.atgt.FrameByFrame(img, time_now) 
                time_now += 1/30
